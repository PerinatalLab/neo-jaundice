...
rule REGENIE_step1_conditionalXXX:
        ...
        output:
                temp('results/conditional_GWAS/regenie/step1/resultsXXX/{sample}_1.loco.gz'),
                temp('results/conditional_GWAS/regenie/step1/resultsXXX/{sample}_pred.list')
        params:
...
rule extract_variant:
	...
	shell:
#		'bcftools view {input[0]} -r 2:234627536 -Oz -o {output[0]}'
		'/home/pol.sole.navais/soft/qctool_v2.2.0/qctool -g {input[0]} -incl-rsids {input[1]} -og {output[0]}'

rule top_SNP_vcf_to_BGEN:
	''
	input:
		'results/conditional_GWAS/regenie/BGEN/temp/fets_rs6755571.vcf'
	output:
		'xx'
	shell:
		'/home/pol.sole.navais/soft/qctool_v2.2.0/qctool -g {input[0]} -incl-rsids {input[1]} -og {output[0]}'
...
rule REGENIE_step2_conditionalXXX:
        'Whole genome regression model is fit to the traits.'
        input:
        ...
        output:
                temp(expand('results/conditional_GWAS/regenie/step2/tempXXX/{{sample}}/{{CHR}}_{pheno}.regenie', pheno= pheno_file['phenotypes']))
        params:
